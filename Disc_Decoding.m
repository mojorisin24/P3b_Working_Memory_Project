...
%Modified by Carlos Carrasco (UC Davis) to accomodate P3 oddball
%dataset.
...
svmECOC.nBins = 4; % # of stimulus bins
...
% here, we removed external EOG electrodes & mastoid ref, for 27 electrodes
        %Getting rid of interpolated channels for relevant subs. 

    if subs(s) == 2 
        ReleventChan = ReleventChan(ReleventChan ~= 1); 
        ReleventChan = ReleventChan(ReleventChan ~= 17);
...
nSamps = length(svmECOC.time);
    ...
    dataLocation = [pwd '/Decoding_Oddball_Prepped/']; % set directory of data set
    loadThis = strcat(dataLocation,'/Decoding_DO_',currentSub,'.mat');
    load(loadThis)
    
    saveDir = [pwd '/Disc_Results_Angles'];
    ...
    if cond == 1 %rare cardinal
        eegs = binorgEEG.binwise_data([1 2 3 4 ]);
        nPerBin = binorgEEG.n_trials_this_bin([1 2 3 4 ]);
    elseif cond == 2 %freq cardinal
        eegs = binorgEEG.binwise_data([5 6 7 8 ]);
        nPerBin = binorgEEG.n_trials_this_bin([1 2 3 4 ]); %flooring using the rarebins 
    elseif cond == 3 %rare diagonal
        eegs = binorgEEG.binwise_data([9 10 11 12]);
        nPerBin = binorgEEG.n_trials_this_bin([9 10 11 12]); %flooring using the rarebins 
    elseif cond == 4 %freq diagonal 
        eegs = binorgEEG.binwise_data([13 14 15 16]);
        nPerBin = binorgEEG.n_trials_this_bin([ 9 10 11 12]); %flooring using the rarebins 
    end 
    ...
    if cond == 1 
        OutputfName = strcat(saveDir,'/Disc_Rare_Cardinal_Decoding_',currentSub,'.mat');
    elseif cond == 2 
        OutputfName = strcat(saveDir,'/Disc_Freq_Cardinal_Decoding_',currentSub,'.mat');
    elseif cond == 3 
        OutputfName = strcat(saveDir,'/Disc_Rare_Diagonal_Decoding_',currentSub,'.mat');
    elseif cond == 4 
        OutputfName = strcat(saveDir,'/Disc_Freq_Diagonal_Decoding_',currentSub,'.mat');
    end 
